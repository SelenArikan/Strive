    ...
    <div className="relative flex min-h-screen w-full flex-col bg-background-dark font-display text-white overflow-x-hidden antialiased selection:bg-primary selection:text-white">
      ...
      <header className="sticky top-0 z-50 flex items-center justify-between whitespace-nowrap border-b border-solid border-[#393028] bg-[#181411]/90 backdrop-blur-md px-4 sm:px-10 py-3">
        ...
        <div className="flex flex-1 justify-end gap-4 lg:gap-6 items-center">
          {/* Search - Desktop only */}
          <div className="hidden lg:flex w-full max-w-[300px]">
            <div className="flex w-full items-center rounded-full bg-[#2a231d] px-4 py-2 ring-1 ring-[#393028] focus-within:ring-primary transition-all">
              <span className="material-symbols-outlined text-[#baab9c]">search</span>
              <input className="w-full bg-transparent px-2 text-sm text-white placeholder-[#baab9c] focus:outline-none" placeholder={t('common.search')} />
            </div>
          </div>
      ...
      <main className="flex-grow flex flex-col">
        {/* Hero Section */}
        <section className="relative min-h-[60vh] w-full overflow-hidden bg-[#181411]">
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-[#181411] via-[#181411]/80 to-transparent"></div>
          <div className="relative z-10 mx-auto flex h-full max-w-[1400px] flex-col justify-center px-6 sm:px-8 py-20 lg:flex-row lg:items-center">
            <div className="flex flex-1 flex-col justify-center space-y-6 lg:max-w-xl">
              <div className="flex items-center gap-2">
                <span className="inline-block h-px w-8 bg-primary"></span>
                <span className="text-primary text-sm font-bold tracking-[0.2em] uppercase">{t('shop.newArrivals')}</span>
              </div>
              <h1 className="font-display text-4xl sm:text-5xl lg:text-7xl font-black italic uppercase leading-none tracking-tighter text-white">
                {t('shop.unleashTitle1')} <br />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-white to-gray-500">{t('shop.unleashTitle2')}</span>
              </h1>
              ...
              <div className="flex flex-wrap gap-4 pt-4">
                <Link
                  href="/shop/catalog"
                  className="flex items-center justify-center rounded-full bg-primary px-6 sm:px-8 py-3 sm:py-4 text-sm sm:text-base font-bold uppercase tracking-wider text-[#181411] hover:bg-white hover:scale-105 transition-all shadow-[0_0_20px_rgba(242,127,13,0.5)]"
                >
                  {t('shop.shopCollection')}
                </Link>
                <button
                  onClick={() => setShowVideoModal(true)}
                  className="flex items-center justify-center rounded-full border border-[#393028] bg-transparent px-6 sm:px-8 py-3 sm:py-4 text-sm sm:text-base font-bold uppercase tracking-wider text-white hover:border-white hover:bg-white/5 transition-all"
                >
                  {t('shop.watchFilm')}
                  <span className="material-symbols-outlined ml-2 text-lg">play_arrow</span>
                </button>
              </div>
            </div>
            {/* Floating Hero Product Image */}
            <div className="relative mt-12 flex flex-1 justify-center lg:mt-0 lg:justify-end">
              <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[300px] sm:w-[500px] h-[300px] sm:h-[500px] bg-primary/20 rounded-full blur-[100px] animate-pulse pointer-events-none"></div>
              <Image
                src="/Gemini_Generated_Image_9mufdw9mufdw9muf.png"
                alt="Elite Pro Basketball"
                width={450}
                height={450}
                className="relative z-10 max-w-[250px] sm:max-w-[300px] md:max-w-[450px] drop-shadow-2xl rotate-12 hover:rotate-0 transition-transform duration-700 ease-out"
        ...
        {/* Sticky Filter Bar */}
        <div className="sticky top-[72px] z-40 w-full border-b border-[#393028] bg-[#181411]/95 backdrop-blur shadow-lg">
          <div className="mx-auto flex max-w-[1400px] items-center justify-between px-4 sm:px-6 py-4 overflow-x-auto">
            <div className="flex items-center gap-2 sm:gap-3 no-scrollbar overflow-x-auto pr-4">
              {[
                ...
                <button
                  ...
                  className={`group flex h-9 shrink-0 items-center justify-center gap-x-2 rounded-full px-4 transition-colors ${activeFilter === filter.key
                    ? 'bg-white text-[#181411]'
                    : 'border border-[#393028] bg-transparent hover:border-primary hover:text-primary'
                    }`}
                >
            ...
            <div className="hidden md:flex items-center gap-2 pl-4 border-l border-[#393028]">
              <span className="text-xs font-medium text-gray-500 uppercase tracking-wider">{t('shop.sortBy')}</span>
              <button className="flex items-center gap-1 text-sm font-medium text-white hover:text-primary">
